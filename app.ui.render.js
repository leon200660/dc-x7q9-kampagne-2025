(function () {
  ...
  function appendDashboardEmptyState(host, text) {
    if (!host) return;
    const shell = document.createElement("div");
    shell.className = "dashboard-empty-state";
    shell.setAttribute("role", "note");

    const badge = document.createElement("span");
    badge.className = "dashboard-empty-state-icon";
  ...
  function createDashboardEntryButton(entry, options) {
    if (!entry) return null;

    const opts = options && typeof options === "object" ? options : {};
    const cat = callIfFn(app.getCategoryById, entry.categoryId);
    const icon = toNonEmptyString(opts.icon) || (cat ? cat.icon || "📁" : "📁");
    const title = toNonEmptyString(entry.title) || "Ohne Titel";
    const prefix = toNonEmptyString(opts.prefix);
    const meta = [];

    if (!opts.hideCategory && cat && cat.name) meta.push(cat.name);
    if (opts.meta) meta.push(opts.meta);

    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = opts.className || "related-link";
    btn.innerHTML = `
      <span aria-hidden="true">${app.escapeHtml(icon)}</span>
      <span class="dashboard-link-copy">
        <strong>${app.escapeHtml(prefix ? `${prefix}: ${title}` : title)}</strong>
        ${meta.length ? `<small class="dashboard-link-meta">${app.escapeHtml(meta.join(" | "))}</small>` : ""}
      </span>
    `;
    btn.addEventListener("click", () => callIfFn(app.navigateToEntry, entry.id));
    return btn;
  ...
  function renderDashboardLatestFocus() {
    const list = ensureDashboardLatestList();
    if (!list) return;
    ...
    if (!recap && !patch) {
      appendDashboardEmptyState(list, "Noch keine Recaps oder Patch Notes gefunden.");
      return;
    }

    const recapBtn = createDashboardEntryButton(recap, {
      prefix: "Letzter Recap",
      icon: "📜",
      hideCategory: true,
    });
    const patchBtn = createDashboardEntryButton(patch, {
      prefix: "Letzter Patch",
      icon: "🧩",
      hideCategory: true,
    });
  ...
  function renderDashboardRecentViews() {
    const dom = app.dom || {};
    if (!dom.dashboardRecentList) return;

    const recent =
      callIfFn(app.getRecentViewedEntries, 5) || callIfFn(app.computeRecentUpdates, 5) || [];
    dom.dashboardRecentList.innerHTML = "";

    if (!recent.length) {
      appendDashboardEmptyState(
        dom.dashboardRecentList,
        "Noch nichts angesehen. Sobald Einträge geöffnet wurden, erscheinen sie hier."
      );
      return;
    }

    recent.forEach((entry) => {
      const btn = createDashboardEntryButton(entry, { icon: "🧭" });
      if (btn) dom.dashboardRecentList.appendChild(btn);
    });
  }

  function renderDashboardThreadLinks() {
    const dom = app.dom || {};
    if (!dom.dashboardThreadLinks) return;

    const counts = callIfFn(app.computeCategoryCounts) || {};
    const preferred = ["npcs", "factions", "cities", "phenomena", "history", "items", "gods"];
    const used = new Set();
    const selected = [];

    const pushCategory = (cat) => {
      if (!cat || used.has(cat.id) || !(counts[cat.id] > 0)) return;
      used.add(cat.id);
      selected.push(cat);
    };

    preferred.forEach((id) => pushCategory(callIfFn(app.getCategoryById, id)));

    const allCats = Array.isArray(app.allCategories) ? app.allCategories : [];
    allCats
      .filter((cat) => cat && cat.id && cat.id !== "all")
      .sort((a, b) => (counts[b.id] || 0) - (counts[a.id] || 0))
      .forEach(pushCategory);

    dom.dashboardThreadLinks.innerHTML = "";

    if (!selected.length) {
      appendDashboardEmptyState(
        dom.dashboardThreadLinks,
        "Sobald mehr Themen im Glossar liegen, erscheinen hier direkte Fäden zum Weiterlesen."
      );
      return;
    }

    selected.slice(0, 4).forEach((cat) => {
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "filter-chip";
      btn.innerHTML = `${app.escapeHtml(cat.icon || "📁")} ${app.escapeHtml(
        cat.name || cat.id
      )} <strong style="margin-left:0.25rem">(${counts[cat.id] || 0})</strong>`;
      btn.addEventListener("click", () => callIfFn(app.navigateToCategory, cat.id));
      dom.dashboardThreadLinks.appendChild(btn);
    });
  }

  function renderDashboardTimelineFocus() {
    const dom = app.dom || {};
    if (!dom.dashboardTimelineHighlights) return;

    const items = callIfFn(app.getTimelineItems) || [];
    const highlights = items.filter(Boolean).slice(-3).reverse();
    dom.dashboardTimelineHighlights.innerHTML = "";

    if (!highlights.length) {
      appendDashboardEmptyState(
        dom.dashboardTimelineHighlights,
        "Noch keine Timeline-Ereignisse hinterlegt."
      );
      return;
    }

    highlights.forEach((item) => {
      const linkedEntry = callIfFn(app.resolveTimelineEntry, item);
      const meta = [toNonEmptyString(item.date), toNonEmptyString(item.meta)].filter(Boolean);
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "related-link";
      btn.innerHTML = `
        <span aria-hidden="true">${app.escapeHtml(toNonEmptyString(item.icon) || "🕰️")}</span>
        <span class="dashboard-link-copy">
          <strong>${app.escapeHtml(toNonEmptyString(item.title) || "Ohne Ereignistitel")}</strong>
          ${meta.length ? `<small class="dashboard-link-meta">${app.escapeHtml(meta.join(" | "))}</small>` : ""}
        </span>
      `;
      btn.addEventListener("click", () => {
        if (linkedEntry && linkedEntry.id) {
          callIfFn(app.navigateToEntry, linkedEntry.id);
          return;
        }
        callIfFn(app.navigateToTimeline);
      });
      dom.dashboardTimelineHighlights.appendChild(btn);
    });
  }

  function renderDashboardUpdatedEntries() {
    const dom = app.dom || {};
    if (!dom.dashboardUpdatedList) return;

    const recent = callIfFn(app.computeRecentUpdates, 5) || [];
    dom.dashboardUpdatedList.innerHTML = "";

    if (!recent.length) {
      appendDashboardEmptyState(
        dom.dashboardUpdatedList,
        "Noch keine zuletzt geänderten Einträge gefunden."
      );
      return;
    }

    recent.forEach((entry) => {
      const btn = createDashboardEntryButton(entry, { icon: "🛠️" });
      if (btn) dom.dashboardUpdatedList.appendChild(btn);
    });
  }

  function collapseDashboardText(raw) {
    return toNonEmptyString(raw).replace(/\s+/g, " ").trim();
  }

  function truncateDashboardText(raw, maxLength) {
    const text = collapseDashboardText(raw);
    const limit = typeof maxLength === "number" && maxLength > 0 ? Math.floor(maxLength) : 220;
  ...
  function getDashboardRegionFocus(cfg) {
    const notes = Array.isArray(cfg && cfg.notes) ? cfg.notes : [];
    const line = notes.map((value) => toNonEmptyString(value)).find((value) => /^region(?:en)?-fokus\s*:/i.test(value));
    if (!line) return "";
    return line.replace(/^region(?:en)?-fokus\s*:/i, "").trim().replace(/[.]+$/g, "");
  }

  function getDashboardRegionTerms(regionFocus) {
    return toNonEmptyString(regionFocus)
      .split(/\s*(?:,|&|\/|\bund\b)\s*/i)
      .map((value) => collapseDashboardText(value).toLowerCase())
      .filter((value) => value.length >= 3);
  }

  function getDashboardWorldFocus(cfg) {
    const visibleEntries =
      callIfFn(app.getVisibleEntries) ||
      (Array.isArray(app.entries) ? app.entries.filter(Boolean) : []);
    const recent = callIfFn(app.computeRecentUpdates, 24) || [];
    const preferredCategories = new Set(["cities", "factions", "history", "phenomena", "npcs", "gods", "items"]);
    const excludedCategories = new Set(["recaps", "patch-notes", "tutorials", "quests-success", "quests-failed"]);
    const regionFocus = getDashboardRegionFocus(cfg);
    const regionTerms = getDashboardRegionTerms(regionFocus);
    const ordered = [];
    const seen = new Set();

    const pushEntry = (entry) => {
      if (!entry || !entry.id || seen.has(entry.id) || entry.visible === false) return;
      seen.add(entry.id);
      ordered.push(entry);
    };

    recent.forEach(pushEntry);
    visibleEntries.forEach(pushEntry);

    const hasReadableText = (entry) =>
      !!(entry && (collapseDashboardText(entry.summary) || collapseDashboardText(entry.body) || collapseDashboardText(entry.title)));

    const matchesRegion = (entry) => {
      if (!regionTerms.length || !entry) return false;
      const haystack = [
        entry.title,
        entry.summary,
        entry.body,
      ...
      return regionTerms.some((term) => haystack.includes(term));
    };

    let entry =
      ordered.find((candidate) =>
        candidate &&
        preferredCategories.has(candidate.categoryId) &&
        !excludedCategories.has(candidate.categoryId) &&
        hasReadableText(candidate) &&
        matchesRegion(candidate)
      ) || null;
    ...
    if (!entry) {
      entry = ordered.find((candidate) => candidate && !excludedCategories.has(candidate.categoryId)) || null;
    }

    const category = entry ? callIfFn(app.getCategoryById, entry.categoryId) : null;
    const excerpt = entry
      ? truncateDashboardText(entry.summary || entry.body || entry.title, 240)
      : "Noch sind nicht genug Weltfragmente hinterlegt, um einen Brennpunkt aus der Chronik zu formen.";
    const metaParts = [];

    if (regionFocus) metaParts.push(`Aktueller Fokus: ${regionFocus}`);
    if (category && category.name) metaParts.push(category.name);
    if (entry && entry.session) metaParts.push(entry.session);
    if (entry && entry.lastUpdated) metaParts.push(`Zuletzt ergänzt: ${entry.lastUpdated}`);
  ...
  function renderDashboardWorldFocus(cfg) {
    const dom = app.dom || {};
    if (!dom.dashboardWorldTitle || !dom.dashboardWorldBody) return;

    const focus = getDashboardWorldFocus(cfg);
    const entry = focus.entry || null;
    const category = focus.category || null;

    if (dom.dashboardWorldKicker) {
      dom.dashboardWorldKicker.textContent = focus.regionFocus
        ? `Welt im Fokus: ${focus.regionFocus}`
        : "Welt im Fokus";
    }

    dom.dashboardWorldTitle.textContent = entry && entry.title ? entry.title : "Duria im Fokus";
    dom.dashboardWorldBody.textContent =
      focus.excerpt || "Die Chronik sammelt noch weitere Geschichten, Orte und Konflikte.";

    if (dom.dashboardWorldMeta) {
      dom.dashboardWorldMeta.textContent = focus.meta || "Ein Blick auf den nächsten wichtigen Schauplatz der Kampagne.";
    }

    if (dom.dashboardLinkWorldPrimary) {
      dom.dashboardLinkWorldPrimary.textContent = entry ? "Eintrag öffnen" : "Glossar öffnen";
      dom.dashboardLinkWorldPrimary.onclick = () => {
        if (entry && entry.id) {
          callIfFn(app.navigateToEntry, entry.id);
          return;
        }
        callIfFn(app.navigateToCategory, "all");
      };
    ...
    if (dom.dashboardLinkWorldSecondary) {
      dom.dashboardLinkWorldSecondary.textContent =
        category && category.name ? `${category.name} öffnen` : "Timeline öffnen";
      dom.dashboardLinkWorldSecondary.onclick = () => {
        if (category && category.id) {
          callIfFn(app.navigateToCategory, category.id);
          return;
        }
        callIfFn(app.navigateToTimeline);
      };
  ...
  function renderDashboardHelpCard(cfg) {
    const dom = app.dom || {};
    if (!dom.dashboardHelpTitle || !dom.dashboardHelpLines) return;

    const help = cfg && cfg.help && typeof cfg.help === "object" ? cfg.help : { title: "", lines: [] };
    const title = toNonEmptyString(help.title) || "Frag die DMs";
    const lines = Array.isArray(help.lines)
      ? help.lines.map((line) => toNonEmptyString(line)).filter(Boolean)
      : [];
    const fallbackLines = [
      "Du hast Fragen zu Regeln, Lore oder Homebrew?",
      "Frag deine Spielleitung im Discord oder direkt am Tisch.",
      "Markiere dir Einträge, zu denen du später noch etwas klären möchtest.",
    ];
    const visibleLines = (lines.length ? lines : fallbackLines).slice(0, 3);

    dom.dashboardHelpTitle.textContent = title;
    dom.dashboardHelpLines.innerHTML = visibleLines
      .map(
        (line) =>
          `<p class="dashboard-help-line"><span aria-hidden="true">✦</span><span>${app.escapeHtml(line)}</span></p>`
      )
      .join("");
  }

  function getDashboardHeroTitle() {
    const dom = app.dom || {};
    const title = toNonEmptyString(dom.headerTitle && dom.headerTitle.textContent);
    return title || HEADER_DEFAULT_TITLE;
  }

  function getDashboardHeroSubtitle() {
    const dom = app.dom || {};
    const subtitle = toNonEmptyString(dom.headerSubtitle && dom.headerSubtitle.textContent);
    return subtitle || HEADER_DEFAULT_SUBTITLE;
  }

  function getDashboardHeroIntro(cfg) {
    const help = cfg && cfg.help && typeof cfg.help === "object" ? cfg.help : { title: "", lines: [] };
    const helpTitle = toNonEmptyString(help.title);
    const helpLine = Array.isArray(help.lines) ? toNonEmptyString(help.lines[0]) : "";
    const firstNote = Array.isArray(cfg && cfg.notes) ? toNonEmptyString(cfg.notes[0]) : "";
    const singleNote = toNonEmptyString(cfg && cfg.note);

    if (helpTitle && helpLine) return `${helpTitle}: ${helpLine}`;
    if (helpTitle) return helpTitle;
    if (firstNote) return firstNote;
    if (singleNote) return singleNote;
    return "Schlage die Chronik auf und wähle deinen nächsten Pfad durch Duria.";
  }

  function getDashboardHeroVisibleNotes(cfg, introLine) {
    const intro = toNonEmptyString(introLine);
    const notes = Array.isArray(cfg && cfg.notes)
      ? cfg.notes.map((line) => toNonEmptyString(line)).filter(Boolean)
      : [];

    if (notes.length > 1 && intro && notes[0] === intro) return notes.slice(1);
    if (notes.length && !(notes.length === 1 && intro && notes[0] === intro)) return notes;

    const fallback = toNonEmptyString(cfg && cfg.note);
    if (fallback && fallback !== intro) return [fallback];
    return [];
  }
  ...
  function renderDashboardHeroStats() {
    ...
    if (dom.dashboardHeroEntryCount) {
      const entries = Array.isArray(app.entries) ? app.entries : [];
      dom.dashboardHeroEntryCount.textContent = String(entries.length);
    }

    if (dom.dashboardHeroTimelineCount) {
      const items =
        typeof app.getTimelineItems === "function"
          ? app.getTimelineItems()
          : Array.isArray(state.timeline)
          ? state.timeline
          : state.timeline && Array.isArray(state.timeline.items)
          ? state.timeline.items
          : [];
      dom.dashboardHeroTimelineCount.textContent = String(items.length);
    }

    if (dom.dashboardHeroBestiaryCount) {
      const monsters =
        typeof app.getBestiaryMonsters === "function" ? app.getBestiaryMonsters() : [];
      dom.dashboardHeroBestiaryCount.textContent = String(monsters.length);
    }

    if (dom.dashboardHeroCraftingCount) {
      const professions =
        typeof app.getCraftingProfessions === "function" ? app.getCraftingProfessions() : [];
      dom.dashboardHeroCraftingCount.textContent = String(professions.length);
    }
  }
  ...
  function renderDashboard() {
    ...
    if (dom.dashboardProgressValue) {
      dom.dashboardProgressValue.textContent = `${Math.round(percent)}%`;
    }

    if (dom.dashboardHeroTitle) dom.dashboardHeroTitle.textContent = getDashboardHeroTitle();
    if (dom.dashboardHeroSubtitle) dom.dashboardHeroSubtitle.textContent = getDashboardHeroSubtitle();

    const heroIntro = getDashboardHeroIntro(cfg);
    if (dom.dashboardHeroIntro) dom.dashboardHeroIntro.textContent = heroIntro;

    if (dom.dashboardProgressLabel) {
      dom.dashboardProgressLabel.textContent =
        cfg.progressLabel || "Der aktuelle Reisestand wird noch ergänzt.";
    }

    if (dom.dashboardNote) {
      const notes = getDashboardHeroVisibleNotes(cfg, heroIntro);
      if (notes.length) {
        dom.dashboardNote.innerHTML = notes.map((line) => `<div>${app.escapeHtml(line)}</div>`).join("");
      } else {
        dom.dashboardNote.textContent = "Alle Wege führen zurück in die Chronik.";
      }
    }
    ...
    renderDashboardLatestFocus();
    renderDashboardRecentViews();
    renderDashboardThreadLinks();
    renderDashboardTimelineFocus();
    renderDashboardUpdatedEntries();
    renderDashboardWorldFocus(cfg);
    renderDashboardHelpCard(cfg);
