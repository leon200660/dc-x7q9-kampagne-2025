...
(function () {
  ...
  const dom = {
    ...
    heroesKicker: document.getElementById("heroes-kicker"),
    heroesTitle: document.getElementById("heroes-title"),
    heroesSubtitle: document.getElementById("heroes-subtitle"),
    heroesRecognition: document.getElementById("heroes-recognition"),
    heroesSelector: document.getElementById("heroes-selector"),
    heroesContent: document.getElementById("heroes-content"),
    arenaView: document.getElementById("arena-view"),
    craftingView: document.getElementById("crafting-view"),
    craftingTitle: document.getElementById("crafting-title"),
    craftingSubtitle: document.getElementById("crafting-subtitle"),
    craftingSearch: document.getElementById("crafting-search"),
    craftingPositionChip: document.getElementById("crafting-position-chip"),
    craftingMeta: document.getElementById("crafting-meta"),
    craftingJumpbar: document.getElementById("crafting-jumpbar"),
    craftingContent: document.getElementById("crafting-content"),
    craftingOverviewButton: document.getElementById("crafting-overview-button"),
    ...
    dashboardHeroSubtitle: document.getElementById("dashboard-hero-subtitle"),
    dashboardHeroIntro: document.getElementById("dashboard-hero-intro"),
    dashboardProgressValue: document.getElementById("dashboard-progress-value"),
    dashboardHeroEntryCount: document.getElementById("dashboard-hero-entry-count"),
    dashboardHeroTimelineCount: document.getElementById("dashboard-hero-timeline-count"),
    dashboardHeroBestiaryCount: document.getElementById("dashboard-hero-bestiary-count"),
    dashboardHeroCraftingCount: document.getElementById("dashboard-hero-crafting-count"),
    ...
    dashboardLinkGlossary: document.getElementById("dashboard-link-glossary"),
    dashboardLinkTimeline: document.getElementById("dashboard-link-timeline"),
    dashboardLinkTimelinePrimary: document.getElementById("dashboard-link-timeline-primary"),
    dashboardLinkCrafting: document.getElementById("dashboard-link-crafting"),
    dashboardLinkBestiaryPrimary: document.getElementById("dashboard-link-bestiary-primary"),
    dashboardTimelineHighlights: document.getElementById("dashboard-timeline-highlights"),
    dashboardThreadLinks: document.getElementById("dashboard-thread-links"),
    dashboardWorldKicker: document.getElementById("dashboard-world-kicker"),
    dashboardWorldTitle: document.getElementById("dashboard-world-title"),
    dashboardWorldBody: document.getElementById("dashboard-world-body"),
    dashboardWorldMeta: document.getElementById("dashboard-world-meta"),
    dashboardLinkWorldPrimary: document.getElementById("dashboard-link-world-primary"),
    dashboardLinkWorldSecondary: document.getElementById("dashboard-link-world-secondary"),
    dashboardHelpTitle: document.getElementById("dashboard-help-title"),
    dashboardHelpLines: document.getElementById("dashboard-help-lines"),
    dashboardLinkHelpGlossary: document.getElementById("dashboard-link-help-glossary"),
    dashboardLinkHelpCrafting: document.getElementById("dashboard-link-help-crafting"),

    dashboardRecentList: document.getElementById("dashboard-recent-list"),
    dashboardUpdatedList: document.getElementById("dashboard-updated-list"),
  ...
  function encodeRouteToken(value) {
    if (typeof app.encodeRouteToken === "function") return app.encodeRouteToken(value);
    try {
      return encodeURIComponent(toNonEmptyString(value));
    } catch {
      return toNonEmptyString(value);
    }
  }

  function getCraftingOverviewHash() {
    return "#handwerk";
  }

  function getCraftingProfessionHash(id) {
    const token = encodeRouteToken(id);
    return token ? `#handwerk-${token}` : "#handwerk";
  }

  function getHeroesOverviewHash() {
    return "#heldenakte";
  }

  function getHeroRecordHash(id) {
    const token = encodeRouteToken(id);
    return token ? `#heldenakte?hero=${token}` : "#heldenakte";
  }

  function getArenaOverviewHash() {
    return "#kampfprobe";
  }

  function getArenaBattleHash(heroId, monsterId) {
    const params = [];
    const heroToken = encodeRouteToken(heroId);
    const monsterToken = encodeRouteToken(monsterId);
    if (heroToken) params.push(`hero=${heroToken}`);
    if (monsterToken) params.push(`monster=${monsterToken}`);
    return params.length ? `#kampfprobe?${params.join("&")}` : "#kampfprobe";
  }

  function renderViewBadge() {
    if (!dom.headerBadge) return;
    ...
    dom.headerBadge.textContent = labels[current] || "";
  }
  ...
  function setView(view) {
    ...
    if (dom.heroesView) dom.heroesView.hidden = next !== "heroes";
    if (dom.arenaView) dom.arenaView.hidden = next !== "arena";
    if (dom.craftingView) dom.craftingView.hidden = next !== "crafting";
    ...
    if (dom.app) {
      ...
      dom.app.classList.toggle("view-heroes", next === "heroes");
      dom.app.classList.toggle("view-arena", next === "arena");
      dom.app.classList.toggle("view-crafting", next === "crafting");
      dom.app.classList.toggle("view-bestiary", next === "bestiary");
      if (next !== "bestiary") {
        dom.app.classList.remove("bestiary-detail-open", "bestiary-list-open");
      }
    }
    ...
    if (next === "heroes") callIfFn(app.renderHeroes);
    if (next === "arena") callIfFn(app.renderArena);
    if (next === "crafting") callIfFn(app.renderCrafting);
  ...
  function navigateToCrafting(options) {
    const opts = options && typeof options === "object" ? options : {};
    const hash = typeof app.getCraftingOverviewHash === "function" ? app.getCraftingOverviewHash() : "#handwerk";
    window.location.hash = hash;
    if (!opts.keepScroll) window.scrollTo(0, 0);
  }

  function navigateToHeroes(options) {
    const opts = options && typeof options === "object" ? options : {};
    const hash =
      typeof app.getHeroesOverviewHash === "function" ? app.getHeroesOverviewHash() : "#heldenakte";
    window.location.hash = hash;
    if (!opts.keepScroll) window.scrollTo(0, 0);
  }
  ...
  function navigateToArena(options) {
    const opts = options && typeof options === "object" ? options : {};
    const hash =
      typeof app.getArenaOverviewHash === "function" ? app.getArenaOverviewHash() : "#kampfprobe";
    window.location.hash = hash;
    if (!opts.keepScroll) window.scrollTo(0, 0);
  }
  ...
  function bindDashboardQuickActionsOnce() {
    ...
    bind(dom.dashboardLinkGlossary, () => navigateToCategory("all"));
    ...
    bind(dom.dashboardLinkTimelinePrimary, () => {
      window.location.hash = "#timeline";
    });

    bind(dom.dashboardLinkCrafting, () => {
      window.location.hash = "#handwerk";
    });

    bind(dom.dashboardLinkBestiaryPrimary, () => {
      window.location.hash = "#bestiary";
    });

    bind(dom.dashboardLinkHelpGlossary, () => navigateToCategory("all"));
    bind(dom.dashboardLinkHelpCrafting, () => {
      window.location.hash = "#handwerk";
    });
  }
  ...
  function bindFabNavOnce() {
    ...
    bind(dom.navHeroes, () => {
      window.location.hash = "#heldenakte";
    });

    bind(dom.navCrafting, () => {
      window.location.hash = "#handwerk";
    });

    bind(dom.navArena, () => {
      window.location.hash = "#kampfprobe";
    });
  }
  ...
  Object.assign(app, {
    ...
    getCurrentView,
    renderViewBadge,
    getCraftingOverviewHash,
    getCraftingProfessionHash,
    getHeroesOverviewHash,
    getHeroRecordHash,
    getArenaOverviewHash,
    getArenaBattleHash,
